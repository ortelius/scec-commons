...
func TestUsersForGroup(t *testing.T) {
	...
	users4grp2nft.MarshalNFT(cid2json)      // generate the cid and nft json for user object
	// fmt.Printf("%s=%s\n", users4grp2nft.Key, users4grp2nft.NftJSON)
	assert.Equal(t, expected, users4grp2nft.NftJSON, "check nft json against expected results")

	var nft2users4grp UsersForGroup // define user object to marshal into

	nft2users4grp.NftJSON = expected     // set the nft json
	nft2users4grp.UnmarshalNFT(cid2json) // convert the json string into the user object
	nft2users4grp.MarshalNFT(cid2json)   // recalcuate the cid and nft json for the new user object
	assert.Equal(t, expected, nft2users4grp.NftJSON, "check unmarshalled against expected results")
